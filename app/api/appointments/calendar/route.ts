...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!userId) {
      ...
      if (data.userEmail) {
        user = await db.user.findUnique({
          where: { emailHash: hashEmail(data.userEmail) },
        })
      }
      ...
      if (!user) {
        ...
        if (data.userPhone && user.phone !== data.userPhone)
          updateData.phone = data.userPhone
    ...
    console.log(
      '[CREATE BOOKING] Created booking:',
      JSON.stringify(
        {
          id: booking.id,
