...
async function testImages() {
  ...
  users.forEach(user => {
    console.log(`- ${user.name || user.email}: role=${user.role}, image=${user.image || 'NULL'}, created=${user.createdAt}`)
  })
  ...
  if (barbersWithoutImage.length > 0) {
    ...
    for (const barber of barbersWithoutImage) {
      await prisma.user.update({
        ...
        data: { image: 'https://lh3.googleusercontent.com/a/default-google-avatar' }
      })
